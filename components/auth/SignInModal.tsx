...
import { X, Eye, EyeOff, Zap, Lock, Key, Activity } from 'lucide-react'
...
export function SignInModal({ open, onOpenChange, onSwitchToSignUp, onSwitchToForgotPassword }: SignInModalProps) {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Access Granted: Connection Established')
      onOpenChange(false)
      setEmail('')
      setPassword('')
    } catch (error: any) {
      ...
      >
        ...
        <div className="flex flex-col md:flex-row h-auto md:h-[620px]">
          ...
          <div className="hidden md:flex md:w-1/2 bg-slate-950 relative overflow-hidden border-r border-white/10">
            ...
            <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-brand-accent/20 blur-[100px] rounded-full" />
            ...
            <div className="relative z-10 flex flex-col justify-between p-12 w-full">
              ...
              <div className="space-y-6">
                ...
                <h2 className="text-4xl font-black text-white leading-tight tracking-tighter uppercase italic">
                  Reconnect to <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-accent to-blue-400">Inventory.</span>
                </h2>
                <p className="text-sm font-medium text-slate-400 leading-relaxed max-w-xs">
                  Resync your profile to continue accessing high-performance hardware and secured transactions.
                </p>
              </div>

              <div className="flex items-center gap-4 text-slate-600">
                <Lock className="h-5 w-5" />
                <span className="text-[10px] font-black uppercase tracking-[0.4em]">AES-256 Secured</span>
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-1/2 p-8 md:p-12 relative flex flex-col justify-center bg-[#020617]">
            <div className="max-w-sm mx-auto w-full">
              <div className="mb-8">
                <h1 className="text-2xl font-black text-white tracking-tighter uppercase italic">Access Identity</h1>
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Provide credentials to connect</p>
              </div>

              <form onSubmit={handleSignIn} className="space-y-4">
                <div className="space-y-1.5">
                  <Label htmlFor="email" className="text-[10px] font-black uppercase tracking-widest text-slate-400">Operator Email</Label>
                  <div className="relative group">
                    <Input
                      ...
                      required
                      className="bg-white/5 border-white/10 rounded-xl focus:ring-brand-accent/20 focus:border-brand-accent text-white h-11 pl-4"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="password" className="text-[10px] font-black uppercase tracking-widest text-slate-400">Secret Key</Label>
                    <button
                      type="button"
                      onClick={onSwitchToForgotPassword}
                      className="text-[9px] font-black uppercase tracking-widest text-brand-accent hover:underline transition-all"
                    >
                      Key Recovery
                    </button>
                  </div>
                  <div className="relative">
                    <Input
                      ...
                      placeholder="••••••••"
                      required
                      className="bg-white/5 border-white/10 rounded-xl focus:ring-brand-accent/20 focus:border-brand-accent text-white h-11 pr-10"
                    />
                    <button
                      type="button"
                ...
                <Button 
                  type="submit" 
                  disabled={loading}
                  className="w-full bg-brand-accent hover:bg-brand-accent/90 text-white font-black uppercase tracking-widest rounded-xl h-12 shadow-lg shadow-brand-accent/20 mt-2"
                >
                  {loading ? 'Establishing Link...' : 'Grant Access'}
                </Button>
              </form>
              ...
              <div className="relative my-8">
                ...
                <div className="relative flex justify-center text-[9px] uppercase tracking-[0.3em] font-black text-slate-600">
                  <span className="bg-[#020617] px-4">Standard Link</span>
                </div>
              </div>
              ...
              <p className="text-center text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-8">
                New to the protocol?{' '}
                <button onClick={onSwitchToSignUp} className="text-brand-accent hover:underline transition-all">
                  Initialize Profile
                </button>
              </p>
            </div>
